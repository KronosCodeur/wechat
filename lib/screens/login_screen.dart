...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Center(
        child: _isLoading?CircularProgressIndicator(color: CupertinoColors.systemBlue,strokeWidth: 2,):SingleChildScrollView(
                  ...
                  children: [
                    ...
                    InkWell(
                      ...
                      child: Container(
                        ...
                        child: Text("Login", style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold,fontSize: 18), textAlign: TextAlign.center,),
                      ),
                    )
                  ],
                ),
  ...
  void login() async {
    ...
    if( _userMail.isEmpty || _userPassword.isEmpty){
      ...
      await authService.loginWithEmailAndPassword(_userMail, _userPassword).then((value) async{
        if (value==true) {
          QuerySnapshot snapshot = await DatabaseService(uid: FirebaseAuth.instance.currentUser!.uid).getUserData(_userMail);
